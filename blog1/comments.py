...
def commentPage(request):
    ...
    if 'next' in url:
        if (request.session['current_num'] + 1) > request.session['page_num']:
            ...
            request.session['page_num_list'] = showPageList(request.session['page_num'], request.session['show_num'],
                                                            current=request.session['current_num'])
        ...
        if request.session['current_num'] % request.session['show_num'] != 1:
            ...
            request.session['current_num'] -= 1
            request.session['page_num_list'] = showPageList(request.session['page_num'], request.session['show_num'],
                                                            current=request.session['current_num'], type='pre')
